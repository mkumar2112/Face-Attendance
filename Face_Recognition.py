...
class Face_Recog():
    ...
    def load_data(self):
        ...
        for i in range(len(os.listdir('data'))):
            image_path = f'data/Person_{i+1}/Person.jpg'
            images = face_recognition.load_image_file(image_path)
            # print(images[0])
            face_encoding = face_recognition.face_encodings(images)[0]
            known_face_encodings.append(face_encoding)
            Json_file_path = f'data/Person_{i+1}/data.json'
            face_nameing = Read_Json_File.Extract_Name_from_Json(Json_file_path, 'Name')
            known_face_names.append(face_nameing)
        
        return known_face_encodings, known_face_names
    
    def Face_Verification(self,frame,known_face_encodings, known_face_names):
        # known_face_encodings, known_face_names  =self.load_data()
